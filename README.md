...
- **Usability & risk (important)**: the injection-based gates are verified working in environments **without client-side anti-cheat** (vanilla international / Forge / Fabric / NeoForge, most third-party servers). However, **some servers run their own anti-cheat** (e.g. the NetEase China Edition client scans for injection behavior and reports it to the server) — on those servers the injection **may be detected and your account flagged into a black-room** (with V65 the game itself survives and status reads correctly, but the server-side anti-cheat still black-rooms the account). If you care about your account, use **plain clicker mode** on such servers (leave both gates off = no injection at all = zero game interaction; all clicking features are unaffected)
