...
- **Auto-injection**: after either the attack-only gate or the place-only gate is enabled, a background scan finds javaw/java processes and injects MCCombatStatusJni.dll into every Minecraft Java client not yet injected (identified by GLFW30/LWJGL window class, x64 only, no double injection, re-injects after game restarts, exponential retry backoff 1s→30s on failure); **nothing is injected while both gates are off**. Since DLL V65 the injected module no longer creates a worker thread and never calls AttachCurrentThread (it hooks gdi32!SwapBuffers and reuses the render thread's existing JNIEnv via GetEnv) - verified on the NetEase China Edition 1.20.1 Forge client (game stays alive, status reads correctly; NetEase windows use the same GLFW30 class so auto-injection finds them). The upstream DLL is now **V66** (XOR-string obfuscation / export stripping / PEB unlink + name wipe / PE header erase), and a standalone **injector.exe** (manual mapping: no LoadLibrary, no module-list entry, encrypted embedded payload, nothing written to disk) is included for optional use; the built-in injection still uses the LoadLibrary remote-thread path (verified on real clients, with PEB unlink hiding the module from enumeration)
