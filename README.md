...
- **CPS presets**: one-click 6 / 10 / 15 / 20 / 30 / 40 clicks per second (independent for left/right)
...
- **Cursor-visibility gate**: title-bar cursor-icon toggle; when on, clicking only happens while the system cursor is NOT visible (in-game view) — opening inventory/chat/menus (cursor visible) pauses left/right auto-clicking automatically
- **Auto-injection**: after either the attack-only gate or the place-only gate is enabled, a background scan finds javaw/java processes and injects the status DLL into every Minecraft Java client not yet injected (identified by GLFW30/LWJGL window class, x64 only, no double injection, re-injects after game restarts, exponential retry backoff 1s→30s on failure); **nothing is injected while both gates are off**. Since DLL V65 the injected module no longer creates a worker thread and never calls AttachCurrentThread (it hooks gdi32!SwapBuffers and reuses the render thread's existing JNIEnv via GetEnv) - verified on the NetEase China Edition 1.20.1 Forge client. **Since V67 the injection is manual mapping**: no LoadLibrary, no module-list entry, the DLL is encrypted into the exe and decrypted in memory at runtime - **nothing is ever written to disk**; a standalone injector.exe (same manual mapping) is also included
- **Single-file distribution**: the status DLL is embedded encrypted inside the exe and manual-mapped from memory at runtime (**no %TEMP% extraction anymore**); a sidecar MCCombatStatusJni.dll next to the exe takes priority when present (read into memory, still no disk load / no LoadLibrary)
- **NetEase hygiene**: keep the tool and DLL OFF the desktop (no desktop shortcuts), and start the game first, then the tool - the NetEase client scans desktop files and background-process executables at game startup. Once an account has been flagged (black room), it stays flagged for a while - verify with a clean account
